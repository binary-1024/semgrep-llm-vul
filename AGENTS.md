...
如果对话中出现长期有效的新术语、术语含义调整或容易混淆的中英文概念，优先更新 `docs/glossary.md`。

## 双轨协作模式

项目默认采用“主实现 agent + 研究型 agent”的双轨协作模式来处理关键决策任务。

角色分工：

- 主实现 agent：负责主线实现、仓库修改、测试、benchmark、CLI、验证、提交和最终交付。
- 研究型 agent：负责理论、算法、论文、行业做法、备选方案、失败模式和阶段性建议的梳理。
- 收敛责任：由主实现 agent 汇总研究输出与本地实现/验证结果，形成下一步 Insight、ADR、任务边界或实现计划。

默认启用场景：

- 架构调整或 agent 流程变化。
- 程序分析算法与能力边界选择，例如调用图、污点传播、入口模型、版本对照策略。
...
- 研究型 agent 默认提供理论支撑，不直接承担主线实现责任。
- 研究型 agent 的输出应尽量以“主张 / 适用前提 / 预期收益 / 失败模式 / 最小实验 / kill criterion”收束。
- 主实现 agent 的输出应尽量以“实现变更 / 验证结果 / 残余 unknowns”收束。
- 研究输出不能只停留在聊天中，应优先沉淀到 `docs/Insight/`、`docs/decisions/`、测试、fixture 或 benchmark。
- 双轨协作服务主线推进，做到能支撑下一步决策或验证即可停止，避免研究支线持续膨胀。
- 即使启用了研究型 agent，主实现 agent 仍对最终代码、验证结果和交付质量负责。
