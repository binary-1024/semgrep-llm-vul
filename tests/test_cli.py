...
def test_validate_benchmarks_cli_outputs_inventory(capsys) -> None:
    ...
    assert inventory["summary"]["total"] == 20
    assert inventory["summary"]["candidate"] == 18
...
def test_benchmark_summary_cli_outputs_short_json(capsys) -> None:
    ...
    assert summary["inventory"]["summary"]["total"] == 20
    assert summary["inventory"]["scope"].startswith("case inventory")
    assert summary["inventory_evaluation"]["summary"]["unsupported"] == 8
    assert summary["inventory_evaluation"]["scope"].startswith("M1 sink generation")
    assert summary["executable_suite"]["total"] == 18
