...
- 已引入 M4.3 opt-in live cases，当前覆盖仓库内置 `managed_fixture=open_redirect_pair`、`managed_fixture=open_redirect_meta_refresh_pair` 与 `managed_fixture=open_redirect_refresh_header_pair`。
- 已引入 M4.4 / M4.5 response-level observation 扩展；当前 executable coverage 已包含 open redirect 下的 `meta refresh` body signature 与 `Refresh` response header。
