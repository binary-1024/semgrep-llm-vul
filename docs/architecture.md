...
- 语义边界：第一版只消费 `PocPlan(execution_state=not_run)`；只支持 `http_request_replay` 这一类窄 runner；当前 effect observation 只覆盖 Flask open redirect，可根据 status code 与 `Location` header 给出 `effect_observed`、`effect_not_observed` 或 `effect_unknown`；最终 verdict 只允许 `verified`、`not_verified`、`inconclusive`，并且 `verified` 必须要求 affected 观察到效果且 fixed 未观察到效果；当前支持三类 observation 来源：本地 execution evidence JSON、loopback live HTTP replay，以及仓库内置 managed fixture startup 驱动的 loopback live replay；managed fixture 当前作为内部 runtime 暴露给 pytest/live harness 和 opt-in live benchmark cases，不是公开 CLI startup 接口；live runner 只允许 `localhost` / `127.0.0.1` / `::1`，不自动启动真实项目服务，不跟随 redirect，不连接真实公网目标，不处理 secrets，不执行破坏性 payload。
