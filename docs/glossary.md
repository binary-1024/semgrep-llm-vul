...
结构化 PoC 规划。表示系统已经能把 `reachable=true` 的路径收敛为最小触发输入、请求形态、预期效果、前提条件和 unknowns，但尚未实际执行请求。

当前项目的 M3 第一版默认输出 PoC planning，并把执行状态标记为 `not_run`。

### exp verification

结构化 exp 验证。表示系统已经把 `PocPlan(execution_state=not_run)` 推进成带 affected / fixed 对照的验证结论，并显式保留执行态、效果态和最终 verdict。

当前项目的 M4 第一版默认输出 exp verification report，而不是通用 exploit 执行器结果；M4.1 允许在 loopback live runner 上得到真实本地观察。

### loopback live runner

只连接 `localhost` / `127.0.0.1` / `::1` 上已运行服务的本地 replay runner。

它在本项目中的作用是把真实首跳 HTTP 响应归一化为 `ExecutionEvidenceRecord`，而不是通用 exploit 执行器或环境编排器。

### managed fixture

仓库内置、显式 allowlist 的最小本地服务 runtime。

它的作用是受控地启动/清理 fixture app，并为 live runner 提供 loopback base URL；它不是公开的任意 startup 接口，也不是通用环境编排器。
...
exp 验证与生成。目标是在隔离环境中验证受影响版本和修复版本的行为差异。
...
执行态。用于描述某次 M4 执行观察有没有完成，例如 `completed`、`environment_missing`、`setup_failed`、`run_failed`。

执行态不等于最终漏洞结论。

### effect state

效果态。用于描述某次 M4 执行观察是否真的看到了预期安全效果，例如 `effect_observed`、`effect_not_observed`、`effect_unknown`。

效果态也不等于最终漏洞结论；最终仍需要 affected / fixed 对照。
