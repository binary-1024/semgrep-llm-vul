...
- 研究型 agent 的结论不能只停留在聊天中；需要长期保留的内容应进入 `docs/Insight/`、`docs/decisions/` 或相关规范文档。
- 主实现 agent 对最终分支状态、验证结果和提交内容负责。
- 如果任务同时涉及关键决策与实现，先走 `codex/docs-<topic>` 分支沉淀决策，再基于合入 `main` 的文档开 `codex/feature-<topic>` 分支实现。
