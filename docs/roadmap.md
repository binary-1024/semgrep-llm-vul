...
- 最小 Flask route 入口提取、同文件 helper call chain，以及 direct import 的跨文件一层 helper call chain 已接入 source root fixture。
...
项目已经具备最小 taint path candidate generation、稳定 JSON 报告、CLI 入口、benchmark/case 回归、reachability evidence model 决策、本地 evidence JSON 驱动实现、`reachable=true|false|null` curated cases，以及 Flask route source root 入口提取与跨文件一层 helper 证据。下一步应继续扩大“够用的入口证据”，但仍保持本地、确定性和可验证。
