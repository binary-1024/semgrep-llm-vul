...
M4.2 引入**受控 managed fixture startup**，但第一版先作为**内部 helper**存在，不先扩成新的公开 CLI startup 选项。

第一版边界：

- managed fixture 只允许系统内置、显式枚举的 fixture 对。
...
- 第一消费者是 pytest/live 回归；如确有必要，第二消费者再考虑极少量 opt-in live benchmark case。
...
- 它避免过早把内建 fixture 能力固化成公开 CLI contract。

## 不采用的方案

- 继续只要求外部手动起服务：当前不采用，因为这会让 M4 的真实执行闭环停在半自动状态。
- 立刻新增 `--managed-fixture` 这类公开 startup CLI：当前不采用，因为内建 fixture 语义还应先在 harness 内成熟。
- 允许 `--startup-cmd` 或任意脚本：当前不采用，因为这会直接突破安全边界。
- 直接引入容器或真实项目服务编排：当前不采用，因为这远超本阶段需求。

## 验证方式

- 引入 `managed fixture` 的单元测试或 CLI 间接测试。
- 至少覆盖：
  - `open_redirect_pair` -> `verified`
  - 未知 fixture 名 -> 报错
  - 启停失败 -> `setup_failed` 或受控失败语义
